...
class InsCrawler(Logging):
    ...
    def _dismiss_login_prompt(self):
        ...
        ele_login = self.browser.find_one(".cmbtv .sqdOP")
        if ele_login:
            ele_login.click()

    def _dismiss_alarm_prompt(self):
        ele_later = self.browser.find_one(".mt2GC .aOOlW .HoLwm")
        if ele_later:
            ele_later.click()
        ele_later = self.browser.find_one(".mt3GC .aOOlW .HoLwm")
        if ele_later:
            ele_later.click()
    ...
    def login(self):
        ...
        browser.get(url)
        browser.find_one('input[name="username"]').send_keys('a_e_i_o@naver.com')
        ...
        later_btn = browser.find_one(".cmbtv")
        if later_btn:
            later_btn.click()
    ...
    def get_postsnum_by_tag(self, tag):
        url = "%s/explore/tags/%s/" % (InsCrawler.URL, tag)
        browser = self.browser.get(url)
        time.sleep(30)
        browser.implicitly_wait(1)
        num = browser.find_one(".g47SY")
        return { tag : num }
    ...
    def get_post(self, key):
        ...
        browser.implicitly_wait(1)
        dict = {"key": key}
        fetch_datetime(browser, dict)
        fetch_imgs(browser, dict)
        fetch_likes_plays(browser, dict)
        fetch_caption(browser, dict)
        fetch_comments(browser, dict)
        return dict

    def get_hashtag(self, key):
        browser = self.browser
        browser.get(key)

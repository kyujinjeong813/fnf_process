...
def load_data_and_labels_klp(data_file_dir):
    ...
    for line in lines:
        ...
        if len(line) == 0:
            ...
            lable_split_data = label_data.split(' ')
...
def plot_word_embedding(wv_model_ko):
...
def generate_word_embedding(x_list):
    ...
    config = {
        'min_count' : 0,
        'size' : args.word2vec_dim,
        'sg' : 1, # 0 이면 cbow, 1이면 skip-gram을 사용
        'batch_words' : 1000, # 사전을 구축할 때 한번에 읽을 단어 수
        'iter' : 8, # 너무 오래 걸리면 줄이기
...
def main(args):
    ...
    x_list2, x_pos_list2, x_split_list2, y_list2 = load_data_and_labels_exo('./data_in/EXOBRAIN_NE_CORPUS_10000.txt')

    x_list = x_list + x_list2
    x_pos_list = x_pos_list + x_pos_list2
    x_split_list = x_split_list + x_split_list2
    y_list = y_list + y_list2
    y_list = np.array(y_list)

    char_vocab = build_char_vocab(x_list, threshold=threshold)

    lexicon_list, NER_double_list = load_lexicon_NER('./data_in/gazette/korean_tte')

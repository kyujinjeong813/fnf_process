...
class CNNBiLSTM(nn.Module):
    def __init__(self, vocab_size, char_vocab_size, pos_vocab_size, lex_ner_size, hidden_size,
                 num_layers, embed_size, word2vec, num_classes):
        ...
        self.post_embed = nn.Embedding(pos_vocab_size, embed_size, padding_idx=0)

        self.convs1 = nn.ModuleList([nn.Conv2d(channel_input_word, channel_output, (kernel_size, embed_size))
...
def forward(self, x, x_char, x_pos, x_lex_embedding, lengths):
    ...
    x_word_embedding = self.embed(x)
    trainable_x_word_embedding = self.trainable_embed(x)

    char_output = []
    for i in range(x_char.size(1)):
        x_char_embedding = self.char_embed(x_char[:,i]).unsqeeze(1)
...
def sample(self, x, x_char, x_pos, x_lex_embedding, lengths):
    ...
    char_output = []
    for i in range(x_char.size(1)):
        x_char_embedding = self.char_embed(x_char[:,i].upsqueeze(1)) # (batch, channel_input, words, word_embedding)

        h_convs1 = [F.relu(conv(x_char_embedding)).squeeze(3) for conv in self.convs1]
        h_pools1 = [F.max_pool1d(i, i.size(2)).squeeze(2) for i in h_convs1] # [(batchm channel_out), ...]*len(kernel_sizes)
        h_pools1 = torch.cat(h_pools1, 1) #리스트 쌓아서 tensor화
...
def sample(self, x, x_char, x_pos, x_lex_embedding, lengths):
    ...
    char_output = []
    for i in range(x_char.size(1)):
        x_char_embedding = self.char_embed(x_char[:,i]).unsqueeze(1)

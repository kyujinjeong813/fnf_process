...
from NER.CNN_BiLSTM import CNNBiLSTM
from data_loader import get_loader
from sklearn.metrics import f1_score

predict_NER_dict = {0:'<PAD>', 1:'<START>', 2:'<STOP>', 3:'B_LC', 4:'B_DT',
                    5:'B_OG', 6:'B_TI', 7:'B_PS', 8:'I', 9:'O'}
# 추측컨대... 지명, 날짜, 기관명, 시간, 사람, ?, ?

def parsing_seq2NER(argmax_predictions, x_text_batch):
    predict_NER_list = []
    predict_text_NER_result_batch = copy.deepcopy(x_text_batch[0])
    for argmax_prediction_seq in argmax_predictions:
        ...
        for i, argmax_prediction in enumerate(argmax_prediction_seq):
            ...
            if now_NER_token in ['B_LC', 'B_DT', 'B_OG', 'B_TI', 'B_PS'] and NER_B_flag is None:
                NER_B_flag = now_NER_token
            ...
            elif now_NER_token in ['B_LC', 'B_DT', 'B_OF', 'B_TI', 'B_PS'] and NER_B_flag is not None:
            ...
            elif now_NER_token in ['O'] is not None:
                predict_text_NER_result_batch[i-1] = predict_text_NER_result_batch[i-1] + ':' + prev_NER_token[-2:] + '>'
                NER_B_flag = None
                prev_NER_token = now_NER_token
...
def main(args):
    ...
    # build models
    cnn_bilstm_tagger = CNNBiLSTM(vocab_size=len(vocab), char_vocab_size=len(char_vocab),
                                  pos_vocab_size=len(pos_vocab), embed_size=args.embed_size,
                                  lex_ner_size = len(NER_idx_dic), hidden_size=args.hidden_size,
                                  num_layers=args.num_layers, word2vec=word2vec_matrix, num_classes=10)
    ...
    test_data_loader = get_loader(data_file_dir = args.data_file_dirtest,
                                  vocab=vocab, char_vocab=char_vocab, pos_vocab=pos_vocab,
                                  lex_dict=lex_dict, batch_size=args.test_batch_size,
                                  shuffle=True, num_workers=args.num_workers, dataset='klp')

    # test
    argmax_labels_list = []
    argmax_predictions_list = []

    for step_test, (x_text_batch, x_split_batch, padded_word_tokens_matrix, padded_char_tokens_matrix,
                    padded_pos_tokens_matrix, padded_lex_tokens_matrix, labels, lengths) in enumerate(test_data_loader):
        try:
            ...
            predictions = cnn_bilstm_tagger.sample(padded_word_tokens_matrix, padded_char_tokens_matrix,
            ...
            perdict_NER_list, predict_test_NER_result_batch = parsing_seq2NER(argmax_predictions, x_text_batch)
            label_NER_list, label_text_NER_result_batch = parsing_seq2NER(argmax_labels, x_text_batch)
            ...
            predict_NER_text_string = generate_text_result(predict_test_NER_result_batch, x_split_batch)
            label_text_string = generate_text_result(label_text_NER_result_batch, x_split_batch)
